...
4. 如有非文字证据文件，在证据字段中使用相对路径引用（如 `详见 [截图](./evidence-tc1.jpeg)`）。截图建议使用 `tc<编号>-<描述>.jpeg` 格式命名。
5. 填写汇总表。
...
**结果**：通过
**证据**：测试输出显示 "assert response.status_code == 200" 断言通过。JWT解码后exp声明正确。详见 [测试输出截图](./evidence-tc1.jpeg)
...
**结果**：通过
**证据**：验证层在数据库查询之前即拒绝了请求。详见 [测试输出截图](./evidence-tc2.jpeg)
...
**结果**：通过
**证据**：停止数据库后，通过curl发送请求。收到503响应及正确的错误信息体。测试后重启数据库。详见 [响应截图](./evidence-tc3.jpeg)
