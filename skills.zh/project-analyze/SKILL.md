...
# 原则
- **先观察再推断**：每一步先陈述从代码中能直接观察到的事实，再给出推断。如果推断不出原因，只陈述事实，不编造"为什么"
- **不确定就停下问**：如果无法从代码推断出业务含义，标注"待确认"而非猜测。如果多种解释都合理，呈现给用户选择
- **简单项目简单对待**：如果项目文件不超过 20 个源文件，将 Phase 1/2 阶段合并, 不必过于细分
...
**完成标准**：能回答”这个项目用什么技术、什么架构范式、核心入口在哪”。
...
对于识别到的模式，用一句话说明架构分层规则和数据流向。如果多种模式并存，说明各自的适用范围。
...
**停下来问的时机**：如果核心模块的业务含义无法从代码推断，不要猜测——标注”待确认”并向用户提问。如果模块间依赖关系模糊，列出你观察到的现象，请用户确认。

## Phase 3: 基于分析结果及用户确认，生成 `PROJECT_STATE`

**完成标准**：`PROJECT_STATE/` 目录已创建，所有文档已生成且经用户确认。
...
- **关键设计决策**：仅记录能从代码中明确推断的决策（如”使用 EventBus 解耦模块间通信”）。**禁止编造无法从代码验证的决策理由**；如果推断不出原因，只陈述事实（如”使用 Zustand 做状态管理”），不编造”为什么”。如果某项决策的原因不确定，标注”待确认”。
